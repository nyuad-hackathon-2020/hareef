	...
	class Jet_Smart_Filters_Query_Manager {
		...
		public function get_active_filters_array() {
			...
			foreach ( $request_filters as $filter_id => $filter_data ) {
				...
				$value        = null;
				$hierarchical = ! empty( $filter_data['hierarchical'] ) ? $filter_data['hierarchical'] : false;

				if ( $hierarchical && isset( $filter_data['values'] ) ) {
					$value = $filter_data['values'];
				} elseif ( isset( $active_filters[ $filter_data['query_var'] ] ) ) {
